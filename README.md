...
1. open file [use-actions](./src/ui/hooks/use-actions.ts) and grab the commented code in the bottom of the file
2. create [setup-audio](./src/infra/setup-audio.ts)
3. Setup events `@APP/ANSWER_VALIDATED` and `@APP/GAME_FINISHED` to run the commented code  from [use-actions](./src/ui/hooks/use-actions.ts)

> 👀 If you are facing difficulties on this step take a look at the [proposed solution](https://github.com/saribe/js-nation/blob/step_4/src/infra/setup-audio.ts).
